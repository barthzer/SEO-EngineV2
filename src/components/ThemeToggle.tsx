...
export default function ThemeToggle() {
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-2">
      {/* Sun */}
      <svg
        className={`h-5 w-5 transition-colors duration-300 ${isDark ? "text-text-muted" : "text-amber-400"}`}
      ...
      {/* Moon */}
      <svg
        className={`h-5 w-5 transition-colors duration-300 ${isDark ? "text-accent-pink" : "text-text-muted"}`}
        fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}
      >
